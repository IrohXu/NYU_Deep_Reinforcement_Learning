...
class Critic(nn.Module):
    ...
    def __init__(self, encoder_cfg, num_actions, hidden_dim, hidden_depth,
        ...
        AUGMENTATIONS = {
            ...
            'all':
            nn.Sequential(nn.ReplicationPad2d(image_pad),
                          kornia.augmentation.RandomCrop((84, 84)),  # random shifts is the best for DrQ
                          Intensity(scale=intensity_scale),
                          kornia.augmentation.RandomRotation(degrees=5.0),
                          kornia.augmentation.RandomHorizontalFlip(p=0.5),
                          kornia.augmentation.RandomVerticalFlip(p=0.5),
    ...
    def forward(self, obs, use_aug=False):
        ...
        if self.dueling:
            ...
            v = self.V(obs)
            a = self.A(obs)
            # q = v + (a - torch.max(a))    # force the Q value for the maximizing action to equal V
            q = v + (a - torch.mean(a))    # another method used in Wang et al.’s experiments, use mean
            # END TODO
        else:
            q = self.Q(obs)
...
class DRQLAgent(object):
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, weights,
        ...
        with torch.no_grad():
            ...
            if self.double_q:
                ...
                a_star = next_Q.argmax(dim=1).unsqueeze(1)
                next_Q = next_Q_target.gather(1, a_star)
    ...
    def update(self, replay_buffer, logger, step):
        ...
        if prioritized_replay:
            ...
            prio = np.maximum(np.abs(td_errors), 0.00001)   # 0.00001 denote the \ita
            replay_buffer.update_priorities(idxs, prio)
            # pass
            # End TODO
