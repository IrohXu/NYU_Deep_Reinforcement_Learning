...
class PPO(BasePolicyGradient):
    ...
    def __init__(self, policy_class, env, **hyperparameters):
        ...
        self.critic = policy_class(self.obs_dim, 1)  # output shape is same to the shape of batch_rtgs
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:
            ...
            A_k = batch_rtgs - self.critic(batch_obs)
            ...
            for _ in range(self.n_updates_per_iteration):
                ...
                g = torch.clamp(ratios, min=1-self.epsilon, max=1+self.epsilon)
                A = A_k.detach()
                actor_loss = -torch.min(ratios * A, g * A).mean()
                
                critic_loss = torch.square(batch_rtgs - self.critic(batch_obs)).mean()
