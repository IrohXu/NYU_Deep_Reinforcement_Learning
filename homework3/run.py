...
def train_ppo(args):
    ...
    if args.env == 'Pendulum-v0':
        ...
        total_timesteps = 1205000 # 1005000
        ...
        total_timesteps = 1605000 # 1405000
    ...
    plot_cache = model.return_plot_cache()
    save_cache(args, plot_cache)
...
def save_cache(args, plot_cache):
    table = np.asarray([plot_cache['timesteps'], plot_cache['return'], plot_cache['loss']]).T
    name = str(args.alg) + '_' + str(args.seed) + '_' + str(args.env) + '.csv'

    np.savetxt(os.path.join(SAVE_DIR, name), table, delimiter=",", fmt='%s')
